...
#Set tag
docker tag myvpn-node:latest loiincloud/myvpn-node:v1.2
docker images
docker push loiincloud/myvpn-node:v1.2
...
#Run 
docker run -d -p 9000:3000 loiincloud/myvpn-node:v1.1


docker run -d -p 9000:3000 loiincloud/myvpn-node:latest
